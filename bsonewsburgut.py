...
# Kanallar
SOURCE_CHANNEL = '@burgutuzb'  # Manba kanal
...
def process_text(text):
    """Matnni qayta ishlash: @ bilan boshlanadigan so'zlarni o'chirish va oxiriga qo'shimcha qo'shish"""
    if not text:
        return ""
    ...
    # 2. @ bilan boshlanadigan so'zlarni o'chirish
    pattern = r'\B@\w+\b'
    processed_text = re.sub(pattern, '', text)
    
    # 3. Qo'shimcha bo'shliqlarni tozalash
    processed_text = re.sub(r'\s+', ' ', processed_text).strip()
    
    # 4. Agar matn faqat bo'shliq yoki belgilardan iborat bo'lsa
    if not processed_text or processed_text.isspace():
        return ""
    
    # 5. Matn oxiriga bitta bo'sh qator va qo'shimcha matn qo'shish
...
async def main():
    ...
    print("=" * 50)
    print("QOIDALAR:")
    print("1. Rasm+matn postlari - ✅ KO'CHIRILADI (TO'LIQ + @BSONewsUz matni)")
    print("2. Video+matn postlari - ❌ O'TKAZILADI")
    print("3. GIF+matn postlari - ❌ O'TKAZILADI")
    print("4. Matnda 'Reklama' so'zi - ❌ O'TKAZILADI")
    print("5. @ bilan boshlanadigan so'zlar - ❌ O'CHIRILADI")
    print("6. Rasmga @BSONewsUz matni chiziladi - ✅ O'NG PASTKI BURCHAK")
    print("7. Matn oxiriga - ✅ '👉 @BSOnewsUZ' QO'SHILADI")
    print("=" * 50)
    ...
    async def handler(event):
        try:
            ...
            print(f"📝 Original matn: {original_text[:50]}...")
            
            # 5. Qo'shimcha: @burgutuzb ni @BSONewsUz ga almashtirish
            if processed_text:
                processed_text = processed_text.replace('@burgutuzb', '@BSONewsUz')
                # Yakuniy matnni tekshirish
                if "👉 @BSOnewsUZ" not in processed_text and processed_text.strip():
                    processed_text = f"{processed_text}\n\n👉 @BSOnewsUZ"
                print(f"📝 Yakuniy matn: {processed_text[:100]}...")
                    ...
                    if len(all_messages) > 0:
                        ...
                        if not album_caption.strip():
                            ...
                            if processed_album_text and processed_album_text != "":
                                ...
                                if "👉 @BSOnewsUZ" not in processed_album_text:
                                    processed_album_text = f"{processed_album_text}\n\n👉 @BSOnewsUZ"
